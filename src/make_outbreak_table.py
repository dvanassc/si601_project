...
with sqlite.connect('../HealthDataMap/health.db') as con: 
  cur = con.cursor() 
  cur.execute('''select country, disease, date(report_date) as short_date, count(*)
...
#Dump results into "outbreaks" table that we can subsequently use for outbreak-based analysis  
  ...
  cur.execute("CREATE TABLE outbreaks(country TEXT, disease TEXT, date_short TEXT, report_instances INT)")
